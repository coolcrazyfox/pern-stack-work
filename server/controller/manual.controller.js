...
class ManualController{
    async createManual(req,res){
        try{
            ...
            const newManual =await db.query(`INSERT INTO manual (model,  device, oem_id, analog_oem,  link, image, datetime) values ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
                [
                    model,
                    device,
                    oem_id,
    ...
    async getManualsByOem(req,res){
        try{
            const id = req.query.id
            const manualItems = await db.query(`SELECT * FROM manual WHERE oem_id = $1`, [id])
            res.json(manualItems.rows)
        }catch (e){
            res.status(500).json(e)
            console.error(e.message)
        }
